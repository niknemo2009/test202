...
public class Piligrim {
    ...
    void AddShip(Spaceship ship){
        ...
        if(answer){
            ...
            strAnswer = "Correctly added!";
        }
        //else{
        //    strAnswer = ship.name + " ship wasn't added :(";
        //}
    }
    ...
    boolean FindDublicates(Spaceship ship){
        ...
        if(places.size() > 0){
            for(int i = 0; i < places.size(); i++){
                if(places.get(i).name == ship.name){
                    answer = false;
                    strAnswer = ship.name + " already was added!";
                    i = places.size();
                }
            }
        }
    ...
    boolean HavePlace (){
        ...
        if(places.size() == placeCount){
            answer = false;
            strAnswer = "There is no place for any ships now!";
        }
        return answer;
    }
                    ...
                    case EARTHLING:
                        switch(temp.GetRace()){
                            case TURIAN:
                                answer = false;
                                i = places.size();
                                strAnswer = "You can't add " + ship.GetRace() + ", becouse there is " + temp.GetRace() + " on station";
                            break;
                        }
                        break;
                    case HODAKIAN:
    ...
    boolean CheckWeight(Spaceship ship){
        ...
        if(this.currentWeight + ship.weight > this.maxWeight){
            answer = false;
            strAnswer = ship.name + " is too heavy for station now!";
        }
        return answer;
    }
}
